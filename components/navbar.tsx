...
export default function Navbar() {
  const [activeIndex, setActiveIndex] = useState(0); // Tracks the active nav item

  const navItems = [
    { icon: <HomeIcon />, label: "Home" },
    { icon: <WorkIcon />, label: "Experience" },
    { icon: <ProjectIcon />, label: "Projects" },
    { icon: <EduIcon />, label: "Education" },
    { icon: <SkillsIcon />, label: "Skills" },
    { icon: <ContactIcon />, label: "Contact" },
    { icon: <ResumeIcon />, label: "Resume" },
    ...
    <div className="flex flex-col mx-auto justify-around bg-zinc-800 p-4 rounded-2xl border border-zinc-700">
      {navItems.map((item, index) => (
        <NavPill
          key={item.label}
          icon={item.icon}
          label={item.label.toUpperCase()} // Convert label to uppercase
          isIconLeft={index % 2 === 0} // Alternates icon position
          isActive={index === activeIndex} // Check if the current item is active
          onClick={() => setActiveIndex(index)} // Update the active item on click
        />
      ))}
    </div>
  );
  ...
  return (
    <div
      ...
      className={`py-2 px-8 flex flex-row justify-between items-center rounded-full cursor-pointer transition-colors duration-300 ${
        isActive
          ? "bg-zinc-300 text-zinc-800" // Active styles
          : "hover:bg-zinc-300 hover:text-zinc-800 text-white" // Default hover styles
      }`}
    >
      {isIconLeft ? (
        <>
